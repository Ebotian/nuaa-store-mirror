...
3. 添加文件详情页与下载链接。
4. 构建 Fastify 后端原型，集成索引加载与 REST API。
5. 使用 Vitest 编写 API 注入测试，覆盖健康检查、分类列表、文件详情及错误路径。
6. 部署到 Vercel（作为静态站点 + Serverless API），验证浏览与文件访问。
...
- 在实现阶段会确保：
  - TypeScript 编译通过（tsc）。
  - Fastify API 通过 Vitest（`npm run test`）注入测试，覆盖关键接口合同。
  - Vite 项目能成功构建（npm run build），并在本地打开静态 dist 文件夹能正确加载 index.json 并渲染页面。
