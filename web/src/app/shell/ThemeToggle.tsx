...
export const ThemeToggle = memo(function ThemeToggle() {
	...
	return (
		<button
			...
			aria-pressed={isDark}
			className="group flex items-center gap-3 rounded-full border border-surface-divider bg-surface-elevated/80 px-3 py-2 text-[0.68rem] font-semibold uppercase tracking-[0.3em] text-foreground-muted transition-colors duration-[var(--motion-medium)] hover:border-accent-glow/70 hover:bg-accent-glow/15 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent-focus"
		>
			<span
				className={`relative flex h-7 w-7 items-center justify-center rounded-full transition-colors duration-[var(--motion-medium)] ${
					isDark
