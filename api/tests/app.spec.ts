...
const createCategoriesSnapshot = (
	manifest: ReturnType<typeof createFixtureManifest>
) => manifest.categories.map((category) => ({ ...category }));

describe("Fastify API", () => {
	let app: ReturnType<typeof createApp>;
	const context: FixtureContext = { tempDir: "" };
	...
	beforeEach(async () => {
		...
		await Promise.all([
			writeFile(
				path.join(context.tempDir, "index.json"),
				JSON.stringify(manifest, null, 2),
				"utf-8"
			),
			writeFile(
				path.join(context.tempDir, "categories.json"),
				JSON.stringify(createCategoriesSnapshot(manifest), null, 2),
				"utf-8"
			),
		]);
	...
	it("returns files for a category", async () => {
		...
		const response = await app.inject({
			url: `/api/v1/categories/${categoryId}/files`,
		});
		expect(response.statusCode).toBe(200);
		const payload = response.json();
	...
	it("returns 404 for missing category", async () => {
		const response = await app.inject({
			url: "/api/v1/categories/nonexistent/files",
		});
